...
@section('content')
    ...
    <div class="card card-flush mb-3 mt-3">
        <div class="card-header">
            <div class="card-title">
                <h2 class="fw-bold text-warning">Crear usuario</h2>
            </div>

            <div class="card-toolbar">
                <a href="{{ route('hospital.staff.index') }}"class="btn btn-sm btn-warning btn-active-light-warning">
                    Listado
                </a>
            </div>
        </div>
    ...
    <div class="card p-5">
        <form action="{{ route('hospital.staff.create') }}" method="POST" class="form w-100" novalidate="novalidate"
            ...
            <div class="row mb-3">
                <div class="fv-row col-md-6">
                    ...
                    <input type="text" placeholder="Nombre" name="name" autocomplete="off" class="form-control" />
                </div>
                    ...
                    <select name="role_id" class="form-select form-select-solid">
                        ...
                        @if (isset($roles) && $roles->count() > 0)
                            @foreach ($roles as $row)
                                <option value="{{ $row->id }}">{{ ucwords($row->display_name) }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
