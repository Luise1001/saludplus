...
@section('content')
    ...
    <div class="card p-5">
        <form action="{{ route('hospital.staff.update') }}" method="POST" class="form w-100" novalidate="novalidate"
                    ...
                    <select name="role_id" class="form-select form-select-solid">
                        ...
                        @if (isset($roles) && $roles->count() > 0)
                            @foreach ($roles as $row)
                                <option {{ $user->staff->role_id == $row->id ? 'selected' : '' }}
                                    value="{{ $row->id }}">{{ ucwords($row->display_name) }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
