    ...
    <div class="card mb-5 p-5">
        <div class="d-flex flex-column flex-lg-row mb-5">
            <div class="flex-lg-row-fluid me-0 me-lg-20">
                <form action="{{ route('permission.update') }}" class="form mb-15" method="post" id="kt_careers_form">
                    @csrf
                    @method('PUT')
                    <div class="row mb-5">
                            ...
                            <select name="menu_id" class="form-select form-select-solid">
                                ...
                                @if (isset($menus) && $menus->count() > 0)
                                    @foreach ($menus as $menu)
                                        <option {{ $menu->id == $permission->menu_id ? 'selected' : '' }} value="{{ $menu->id }}">{{ ucwords($menu->name)}}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div class="row mb-5 p-5">
                        <div class="form-check form-check-solid form-switch form-check-custom fv-row">
                            <input class="form-check-input w-45px h-30px" type="checkbox" name="active" value="1" value="1" {{ $permission->active ? 'checked' : '' }}>
